...
meshTexturing::meshTexturing(std::vector<imageFeatures>& features, cameraCalibration& calib, std::string plyFilePath, std::string saveObjFilePath, pcl::PolygonMesh triangles)
{
    ...
    for(imageFeatures feature : features)
    {
        ...
        cam.focal_length_w = calib.getCameraMatrix().at<double>(0, 0);
        cam.focal_length_h = calib.getCameraMatrix().at<double>(1, 1);
        cam.center_w = calib.getPP().x;
        cam.center_h= calib.getPP().y;
